...
We initially preregistered inclusion of quadratic terms for mentalizing to test for the possibility that only extreme low advanced mentalizing is associated with disbelief. After the preregistration but before data collection and analysis, we realized that the polynomial approach was a very poor test of this idea and invites model overfitting [@mcelreathStatisticalRethinkingBayesian2016]. The preregistered models including a quadratic for mentalizing were theoretically dubious and statistically naive, so we left them out of main analyses. We checked a few of the primary models to see if inclusion of a quadratic did much. It did not. Information criteria (WAIC) suggested that models were always better without a quadratic term for mentalizing, and the quadratic term itself never amounted to much. Additional exploration about a possible low-end mentalizing blip in atheism may warrant future research with a statistically appropriate model.
